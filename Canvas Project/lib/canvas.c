...
#define TOTAL_CIRCLES 1500

// Circle Struct
struct Circle {
  int x; // x coordinate
  int y; // z coordinate
  int r; // circles radius
  int cr; // color - RED
  int cg; // color - GREEN
  int cb; // color - BLUE
};

// Circle Anumation Data Struct
...
static struct Circle circles[TOTAL_CIRCLES];
static struct CircleAnimationData animationData[TOTAL_CIRCLES];
...
void initCircles() {
  ...
  // Create circles
  for (int i = 0; i < TOTAL_CIRCLES; i += 1) {
    // Radius
  ...
  // Start JS rendering
  EM_ASM({ render($0, $1) }, TOTAL_CIRCLES * 6, 6);
}
...
struct Circle* getCircles(int canvasWidth, int canvasHeight) {
  ...
  for (int i = 0; i < TOTAL_CIRCLES; i += 1) {
    ...
    // Collision LEFT - set x direction backwards 1
    ...
    // Collision BOTTOM - set y direction backwards 1
